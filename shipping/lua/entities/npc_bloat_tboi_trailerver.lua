...
function ENT:CheckBrim(startpos)
	...
	for k,v in pairs(ents.GetAll()) do
		if (v:IsPlayer() or v:IsNPC() or v:IsNextBot()) and v:GetClass() != "npc_bloat_tboi_trailerver" and v:GetClass() != "ent_bloat_puddle" then
...
function ENT:FindEnemy()
	...
	for k,v in ipairs( _ents ) do
		if ( v:IsPlayer() or v:IsNPC() or v:IsNextBot() and v:GetClass()!="npc_bloat_tboi_trailerver") then
			self:SetEnemy(v)
			return true
		end
	end
	...
	while true do
		...
		if self:HaveEnemy() then
			...
			if self:GetState() == "AttackSlam" then
				...
				if self:GetFrame() > 75 then self:SetState("AttackSlam") end
			end
			...
			if self:GetState() == "AttackCreep" then
				...
				if self:GetFrame() > 56 then self:SetState("AttackCreep") end
			end
		else
...
hook.Add("Think","IncrementFrameBloatTrailerVer",function()
	for k,v in pairs(ents.GetAll()) do
		if v:GetClass() == "npc_bloat_tboi_trailerver" then
...
-- hook.Add( "PostDrawTranslucentRenderables", "BloatDebugTrailervER", function()
...
language.Add("npc_bloat_tboi_trailerver", "LITTLE FUCKER")
killicon.Add("npc_bloat_tboi_trailerver","vgui/hud/killicons/bloatkillicon",Color( 255, 255, 255,255))

end

list.Set( "NPC", "npc_bloat_tboi_trailerver", {
	Name = "Bloat Trailer",
	Class = "npc_bloat_tboi_trailerver",
	Category = "TBOI"
})